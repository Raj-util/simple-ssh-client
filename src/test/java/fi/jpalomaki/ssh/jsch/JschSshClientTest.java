...
public final class JschSshClientTest {
    ...
    @Test
    public void testStderrOverflow() {
        Options options = new Options("0s", "0s", "2B", "4B", "StrictHostKeyChecking=no", false);
        SshClient sshClient = new JschSshClient("src/test/resources/id_rsa_test", "ankka", "/dev/null", options);
        Result result = sshClient.executeCommand(">&2 echo mygodthisisalongstringindeed", userAtHost);
        System.out.println(result.stderrAsText());
    }
